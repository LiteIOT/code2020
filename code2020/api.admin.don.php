...
if ($action == "login") {
    ...
    if ($link) {
        //echo "Connection established.\n";
        $sql = "INSERT INTO $table (don_material, don_quantity, don_donor, don_address) VALUES ('$don_material', '$don_quantity', '$don_donor','$don_address')";        //$sql = "INSERT INTO dbo.userinfo (userName, userPass) VALUES ('$title', '$description')";
...
//end addTest
    ...
    if ($link) {
        ...
        $sql = "SELECT donation.*, product.*, sum(disclosure.dis_quantity) as sum_quantity FROM  
        (donation left join product on donation.product_id = product.product_id) 
        left join disclosure on donation.don_id = disclosure.don_id 
        group by donation.don_id order by donation.don_id desc";
        /*
        $sql = "SELECT donation.*, product.*, sum(disclosure.dis_quantity)
        FROM donation, product left join disclosure on donation.product_id = product.product_id and donation.don_id = disclosure.dis_id
         group by donation.don_id";
         */
