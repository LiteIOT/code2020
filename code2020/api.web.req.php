...
if ($action == "login") {
    ...
    @$req_datetime = date("Y-m-d H:d:i");

    @$req_remain = trim($_REQUEST['quantity']);

    if ($link) {
        //echo "Connection established.\n";
        $sql = "INSERT INTO $table (product_id, req_quantity, req_recipient, req_address, req_datetime, req_remain) VALUES ($product_id, '$req_quantity', '$req_recipient','$req_address', '$req_datetime', $req_remain)";
...
//end addTest
    ...
    if ($link) {
        ...
        $sql = "SELECT * FROM $table order by req_id desc limit 0, 10";
